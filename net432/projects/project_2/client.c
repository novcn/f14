 ...
 *
 *@param channel_name, the name of a channel received after the /switch compmand
...
int subscribe_channel(user * u, char * channel_name){
  ...
  channel * new = NULL;
  channel * current = u->first_channel;
  if(!current){ //join first channel
    u->first_channel = init_channel(u->first_channel);
    *((char*)mempcpy(u->active_channel, channel_name, len)) = '\0';
    *((char*)mempcpy(u->first_channel->channel_name, channel_name, len)) = '\0';
    u->channels_subd++;
    return 1;
  }
  else if(strcmp(current->channel_name, channel_name) == 0){ //already in channel
    return 0;
  } else {
    while(current->next){
      if(strcmp(current->next->channel_name, channel_name) == 0){//already in channel
        *((char*)mempcpy(u->active_channel, channel_name, len)) = '\0';//"switch" to it
        return 0;
      } 
      current = current->next;
    }
    *((char*)mempcpy(u->active_channel, channel_name, len)) = '\0';
    new = init_channel(new); //add new channel to last
    *((char*)mempcpy(new->channel_name, channel_name, len)) = '\0';
    current->next = new;
    u->channels_subd++;
  }
  return 1;
...
int leave_channel(user * u, char * channel_name){
  ...
  if(strcmp(current->channel_name, channel_name) == 0){
    if(strcmp(current->channel_name, u->active_channel) == 0){
      memcpy(u->active_channel, "\0", 1);
    }
    temp = current->next;
    free_channel(current);
    u->first_channel = temp;
    memset(u->active_channel, 0, sizeof(u->active_channel));
  } else {
    while(current->next){
      if(strcmp(current->next->channel_name, channel_name) == 0){
        if(strcmp(current->next->channel_name, u->active_channel) == 0){
          memcpy(u->active_channel, "\0", 1);
        }
        temp = current->next;
...
void parse_input(char * input, struct addrinfo * _result, user * u, int fd_sock){
  ...
  /* ---- SAY ----- SAY ----- SAY ----- SAY ----- SAY ----- */
    ...
    if( (len = strlen(input)) == 0){
      ...
      if(strcmp(u->active_channel, "\0") == 0){
        printf("No channel active, switch or join one!\n");
        return;
      }
